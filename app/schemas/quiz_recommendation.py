...
class RecommendedItemDto(BaseModel):
    ...
    category: str = Field(..., description="카테고리 (TOP, BOTTOM, OUTER, SHOES, ACCESSORY)")
    ...
    position: Optional[ItemPositionDto] = Field(None, description="합성 이미지 내 위치 정보")
...
class OutfitResultDto(BaseModel):
    ...
    composite_image_url: Optional[str] = Field(None, alias="compositeImageUrl", description="합성 이미지 URL")
    image_width: Optional[int] = Field(None, alias="imageWidth", description="이미지 너비 (px)")
    image_height: Optional[int] = Field(None, alias="imageHeight", description="이미지 높이 (px)")
...
class OutfitRecommendationDto(BaseModel):
    ...
    displayOrder: int = Field(..., alias="displayOrder", description="표시 순서 (1, 2, 3)")
    ...
    status: str = Field(default="completed", description="작업 상태 (completed, pending, failed)")
    ...
    completed_at: Optional[str] = Field(None, alias="completedAt", description="완료 시각")
...
class QuizRecommendationResponse(BaseModel):
    ...
    recommended_outfits: List[OutfitRecommendationDto] = Field(
        ...,
        alias="recommendedOutfits",
        description="추천 코디 리스트 (최대 3개)"
    )
                ...
                "recommendedOutfits": [
                    {
                        ...
                        "reason": "깔끔하고 세련된 데이트 룩입니다.",
                        "totalPrice": 250000,
                        "styleBoardUrl": "https://s3.../composite_1.png",
                        "items": [
                            {
                                "productId": 1,
                                "category": "TOP",
